    ...
    constructor( editor ) {
        ...
        fileInput2.addEventListener( 'change', async function () {
            ...
            try {
                ...
                editor.interiorFromJSON( json );
            } catch ( e ) {
                alert( 'Failed To Open Interior' );
                console.error( e );
            } finally {
                form2.reset();
            }
        ...
        option = new UIRow();
        option.setClass( 'option' );
        option.setTextContent( 'Import Interior' );
        option.onClick( function () {
        ...
        option = new UIRow();
        option.setClass( 'option' );
        option.setTextContent( 'Export Interior' );
        option.onClick( function () {
            let data = editor.interiorToJSON();
            const blob = new Blob([JSON.stringify(data, null, 2)], {
                type: 'application/json',
            });
            let url = window.URL.createObjectURL(blob);
            ...
            document.body.appendChild(a);
            a.href = url;
            a.download = editor.scene.name + "_interior.json";
            a.click();
            window.URL.revokeObjectURL(url);
            a.remove();
        } );
